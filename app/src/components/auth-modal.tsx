...
export function AuthModal() {
  ...
  const handleGoogleCredential = useCallback(async (response: { credential: string }) => {
    ...
    try {
      const { data } = await api.post<ApiResponse<AuthResponse>>("/api/auth/google", {
  ...
  function handleGoogleClick() {
    ...
    // Initialize with latest callback before prompting
    initializeGoogle();
      ...
      <Dialog open={authModalOpen} onOpenChange={(open) => { if (!open) handleClose(); }}>
        ...
        <div className="p-6 space-y-4">
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 bg-white border border-slate-200 hover:bg-slate-50 font-bold text-sm py-3 rounded-xl text-slate-700 transition-all shadow-sm disabled:opacity-50"
          >
